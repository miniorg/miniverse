...
async function createSession(this: Session, user: User) {
  user.inbox = [];
  this.set({ user });
}
...
export default class Session extends Universal {
  async signin(fetch: Fetch, username: string, password: string) {
    ...
    const user = await this.fetchActor(fetch, username) as LocalActor;
  ...
  async signup(fetch: Fetch, username: string, password: string, captcha: string) {
    const encoder = new TextEncoder;
    ...
    createSession.call(this, await this.fetchActor(fetch, username) as LocalActor);
  }
}
