...
import { User } from '../lib/store/base';
...
export default function (repository: Repository, port: number): express.Application {
  const application = express();

  application.use(
    express.static('static'),
    (request, _response, next) => {
      (request as Request).repository = repository;
      next();
    },
    (expressRequest, response, next) => {
      const request = expressRequest as Request;
      ...
      asyncAccount.then(async account => {
        if (/^\/bull/i.test(expressRequest.path)) {
          ...
          const actor = account && await account.select('actor');
    ...
    sapper({
      store({ nonce, userActivityStreams }: Request) {
