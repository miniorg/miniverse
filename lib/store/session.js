...
export default {
  async signin(fetch, username, password) {
    ...
    const user = await this.fetchPerson(fetch, username);
  ...
  async signup(fetch, username, password, captcha) {
    const encoder = new TextEncoder;
    ...
    createSession.call(this, await this.fetchPerson(fetch, username));
  }
}
