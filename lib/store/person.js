...
export default {
  ...
  async fetchPerson(fetch, acct) {
    const encodedAcct = encodeURIComponent(acct.includes('@') ?
      acct : `${acct}@${this.get().user['miniverse:webFingerHost']}`);
    ...
    const activityStreams = await fetch(this.get().user.endpoints.proxyUrl, {
      method: 'POST',
      headers: {
        Accept: 'application/activity+json;q=0.9,application/ld+json;q=0.8'
      },
      body: new URLSearchParams({ id: href })
    });
