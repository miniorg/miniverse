...
describe('act', () => {
  ...
  test('performs delete activity', async () => {
    ...
    const note = new Note({
      repository,
      attributedTo: await fabricatePerson(''),
      content: '内容',
      mentions: []
    });
    ...
    await testLoading({
      ...
      await expect(activity.act(note.attributedTo))
        .resolves
        .toBeInstanceOf(Delete);

      await expect(repository.selectRecentNotesByUsernameAndNormalizedHost('', null))
        .resolves
        .toEqual([]);
    });
  });
  ...
  test('performs follow activity', async () => {
    ...
    }, () => activity.act(actor).then(follow => {
  ...
  test('performs undo activity', async () => {
    ...
    await testLoading({
      ...
      await activity.act(actor);

      await expect(repository.selectPersonsIncludingOuterRemoteAccountsByFollowee(object))
        .resolves
        .toEqual([]);
    });
  });
