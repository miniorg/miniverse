...
describe('getHref', () => test('loads and returns href', () => {
  ...
  return testLoading({
    '@context': 'https://www.w3.org/ns/activitystreams',
    href: 'https://ReMoTe.إختبار/person'
  }, () => object.getHref().then(href =>
    expect(href).toBe('https://ReMoTe.إختبار/person')));
}));
...
describe('getTag', () => test('loads and returns tag', () => {
  ...
  return testLoading({
    '@context': 'https://www.w3.org/ns/activitystreams',
    tag: [{ href: 'https://ReMoTe.إختبار/person' }]
  }, () => object.getTag().then(tag =>
    expect(tag[0].getHref()).resolves.toBe('https://ReMoTe.إختبار/person')));
}));
...
describe('act', () => {
  test('performs create activity', async () => {
    ...
    }, () => activity.act(actor.person).then(create => {
      expect(create).toBeInstanceOf(Create);
      expect(create.object).toBeInstanceOf(Note);
      expect(create.object.status.person.id).toBe(actor.person.id);
      expect(create.object.content).toBe('');
      expect(create.object.mentions).toHaveProperty('length', 0);
    }));
  });
  ...
  test('performs delete activity', async () => {
    ...
    await testLoading({
      ...
      await expect(activity.act(note.status.person))
        .resolves
  ...
  test('performs follow activity', async () => {
    ...
    }, () => activity.act(actor.person).then(follow => {
      expect(follow).toBeInstanceOf(Follow);
      expect(follow.actor.id).toBe(actor.person.id);
      expect(follow.object.id).toBe(object.person.id);
    }));
  });
  ...
  test('performs like activity', async () => {
    ...
    }, () => activity.act(actor.person).then(like => {
      expect(like).toBeInstanceOf(Like);
      expect(like.actorId).toBe(actor.person.id);
  ...
  test('performs undo activity', async () => {
    ...
    await testLoading({
      ...
      await activity.act(actor.person);
