...
describe('act', () => {
  test('resolves known URI', async () => {
    ...
    await expect(activity.act(status.actor))
      .resolves
  ...
  test('performs delete activity', async () => {
    ...
    await testLoading({
      ...
      await expect(activity.act(note.status.actor)).resolves.toBe(null);

      await expect(repository.selectRecentStatusesIncludingExtensionsByActorId(note.status.actorId))
        .resolves
        .toEqual([]);
    });
  });
  ...
  test('performs undo activity', async () => {
    ...
    await testLoading({
      ...
      await expect(activity.act(actor.actor)).resolves.toBe(null);
