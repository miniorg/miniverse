...
async function fabricateLocalActor(username) {
  ...
  await repository.insertLocalPerson(actor);
...
async function fabricateLocalObject(username) {
  ...
  await repository.insertLocalPerson(object);
...
async function fabricateRemoteActor(username) {
  ...
  await repository.insertRemotePerson(actor);
...
describe('create', () => {
  test('creates follow', async () => {
    ...
    const properties = await follow.get();

    expect(follow).toBeInstanceOf(Follow);
    expect(follow).toHaveProperty('repository', repository);
    expect(properties).toHaveProperty('actor', actor);
    expect(properties).toHaveProperty('object', object);
    expect((await repository.selectLocalPersonsByFollowee(object))[0])
...
describe('fromParsedActivityStreams', () => {
  test('creates follow from ActivityStreams representation', async () => {
    ...
    expect(properties).toHaveProperty('actor', actor);
    await expect(properties.object).toHaveProperty('id', object.id);
  });
