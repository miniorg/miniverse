...
async function fabricateRemoteActor(username) {
  const actor = new Person({
    ...
    account: new RemoteAccount({
      ...
      publicKey: {
        uri: new URI({ repository, uri: 'https://AcToR.إختبار/#publickey' }),
...
describe('selectPersonByActor', () =>
  test('loads and returns actor person', async () => {
    ...
    follow.actor = new Person({ repository, id: actor.id });

    const result = await follow.selectPersonByActor();
...
describe('selectPersonByObject', () =>
  test('loads and returns object person', async () => {
    ...
    const object = await fabricateLocalObject('被行動者');
    const follow = await fabricateFollow(actor, object);
    follow.object = new Person({ repository, id: object.id });

    const result = await follow.selectPersonByObject();
...
describe('selectRemoteAccountByActor', () =>
  test('loads and returns actor remote account', async () => {
    ...
    const object = await fabricateLocalObject('被行動者');
    const follow = await fabricateFollow(actor, object);
    follow.actor = new Person({ repository, id: follow.actor.id });

    const result = await follow.selectRemoteAccountByActor();

    expect(result).toHaveProperty(
      ['inbox', 'uri', 'id'], actor.account.inbox.uri.id);

    expect(result).toHaveProperty(
      ['publicKey', 'uri', 'id'], actor.account.publicKey.uri.id);

    expect(result).toHaveProperty(['uri', 'id'], actor.account.uri.id);
  }));
...
describe('toActivityStreams', () =>
  test('loads and returns ActivityStreams representation', async () => {
    ...
    const follow = await fabricateFollow(actor, object);
    follow.actor = new Person({ repository, id: follow.actor.id });
    follow.object = new Person({ repository, id: follow.object.id });
...
describe('fromParsedActivityStreams', () => {
  ...
  test('rejects with an instance of RemoteObjectError if object is remote',
    async () => {
      ...
      expect(Follow.fromParsedActivityStreams(repository, actor, activity))
        .rejects
