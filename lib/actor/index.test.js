...
describe('constructor', () => {
  test('assigns itself to its account\'s actor property if present', () => {
    const account = new LocalAccount;
    const actor = new Actor({ repository, account });

    expect(account.actor).toBe(actor);
  });
...
describe('getUri', () => {
  test('loads and returns URI of local account', async () => {
    ...
    actor.account = new LocalAccount({ repository, id: actor.account.id });
  ...
  test('loads and returns URI of remote account', async () => {
    const { actor } =
      await fabricateRemoteAccount({ uri: { uri: 'https://ReMoTe.إختبار/' } });
...
`,
  ...
  test('loads and returns ActivityStreams representation of remote account', async () => {
    ...
    actor.account = new RemoteAccount({ repository, actor });
