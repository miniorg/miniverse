...
describe('createFromHostAndParsedActivityStreams', () => {
  test('creates and resolves with a person', async () => {
    const person = await Person.createFromHostAndParsedActivityStreams(
      repository,
...
`
    ...
    expect(person).toHaveProperty('summary', '');
...
`);
  ...
  test('rejects if type is not Person', () =>
    expect(Person.createFromHostAndParsedActivityStreams(
      repository,
      'finger.remote.xn--kgbechtv',
      new ParsedActivityStreams(repository, {
        '@context': 'https://w3id.org/security/v1',
...
`
  ...
  test('rejects if host of its own ID and ID of public key mismatches', () => {
    return expect(Person.createFromHostAndParsedActivityStreams(
      repository,
      'finger.remote.xn--kgbechtv',
      new ParsedActivityStreams(repository, {
        '@context': 'https://w3id.org/security/v1',
