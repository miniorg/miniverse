...
`
    ...
    const [inboxUriProperties, publicKeyUriProperties] = await Promise.all([
      properties.inbox.uri.get(),
      properties.publicKey.uri.get()
    ]);

    expect(person).toBeInstanceOf(RemotePerson);
    expect(properties).toHaveProperty('username', 'preferred username');
    expect(properties).toHaveProperty('host', 'finger.remote.xn--kgbechtv');
    expect(properties).toHaveProperty('uri', 'https://remote.xn--kgbechtv/@preferred%20username');
    expect(inboxUriProperties).toEqual({
      uri: 'https://remote.xn--kgbechtv/@preferred%20username/inbox'
    });
    expect(publicKeyUriProperties).toEqual({
...
`
      ...
      return expect(person.get()).resolves.toHaveProperty(
          'uri', 'https://remote.xn--kgbechtv/@preferred%20username');
    } finally {
      nock.cleanAll();
    }
  });
