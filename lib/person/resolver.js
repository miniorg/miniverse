...
export default {
  async resolveByUsernameAndNormalizedHost(repository, username, normalizedHost) {
    if (normalizedHost) {
      ...
      const activityStreams = new ActivityStreams(href, AnyHost);
  ...
  async resolveByKeyUri(repository, keyUri) {
    ...
    const key = new ActivityStreams(keyUri, AnyHost);
    const [[owner, host]] = await Promise.all([
      ...
      key.getType(repository).then(type => {
        if (!type.has('Key')) {
          throw new Error;
        }
      })
    ...
    return this.fromActivityStreams(repository, host, owner);
  }
};
