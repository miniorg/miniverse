...
export default class {
  ...
  static async create(repository, object) {
    ...
    const objectProperties = await object.get();
    const [objectObjectProperties, objectActorProperties] = await Promise.all([
      objectProperties.object.get(),
      objectProperties.actor.get()
    ]);

    if (!objectObjectProperties.host && objectActorProperties.host) {
