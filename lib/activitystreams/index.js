...
function load(repository) {
  if (typeof this.body == 'string') {
    this.body = this.resolver.resolve(repository, this.body).then(
      ({ resolver, object }) => {
...
function getChild(body) {
  return body &&
    new this.constructor(body, this.normalizedHost, this.resolver);
}
...
export default class {
  ...
  async getOwner(repository) {
    const { owner } = await load.call(this, repository);
    return getChild.call(this, owner);
  }
