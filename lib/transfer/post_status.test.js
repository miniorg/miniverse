...
test('posts note to inboxes of remote followers', async () => {
  ...
  const { object } = await fabricateFollow({ actor });
  const note = await fabricateNote({ status: { actor: object } });
...
test('deduplicates remote inboxes', async () => {
  ...
  const object = await fabricateLocalAccount();

  await Promise.all(actors.map(actor =>
    fabricateFollow({ actor, object: object.actor })));

  const note = await fabricateNote({ status: { actor: object.actor } });
  await postStatus(repository, note.status);
