/*
  Copyright (C) 2018  Miniverse authors

  This program is free software: you can redistribute it and/or modify
  it under the terms of the GNU Affero General Public License as published by
  the Free Software Foundation, version 3 of the License.

  This program is distributed in the hope that it will be useful,
  but WITHOUT ANY WARRANTY; without even the implied warranty of
  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
  GNU Affero General Public License for more details.

  You should have received a copy of the GNU Affero General Public License
  along with this program.  If not, see <https://www.gnu.org/licenses/>.
*/

import Person from './person';
import RemoteAccount from './remote_account';
import repository from './test_repository';
import URI from './uri';

describe('constructor', () => {
  test('assigns itself to its person\'s account property if present', () => {
    const person = new Person;
    const account = new RemoteAccount({ repository, person });

    expect(person.account).toBe(account);
  });

  test('works without person property', () => {
    new RemoteAccount;
  });
});

describe('selectURI', () => test('loads and returns URI', async () => {
  const account = new RemoteAccount({
    repository,
    inbox: { uri: new URI({ repository, uri: '' }) },
    person: new Person({
      repository,
      username: '',
      host: 'remote.xn--kgbechtv'
    }),
    publicKey: {
      uri: new URI({ repository, uri: '' }),
      publicKeyPem: ''
    },
    uri: new URI({ repository, uri: 'https://ReMoTe.xn--kgbechtv/' })
  });

  await repository.insertRemoteAccount(account);
  account.uri = new URI({ repository, id: account.uri.id });

  await expect(account.selectURI())
    .resolves
    .toHaveProperty('uri', 'https://ReMoTe.xn--kgbechtv/');
}));

describe('selectPerson', () => test('loads and returns person', async () => {
  const account = new RemoteAccount({
    repository,
    inbox: { uri: new URI({ repository, uri: '' }) },
    person: new Person({
      repository,
      username: 'ユーザー名',
      host: 'remote.xn--kgbechtv'
    }),
    publicKey: {
      uri: new URI({ repository, uri: '' }),
      publicKeyPem: ''
    },
    uri: new URI({ repository, uri: '' })
  });

  await repository.insertRemoteAccount(account);
  account.person = new Person({ repository, account, id: account.person.id });

  await expect(account.selectPerson())
    .resolves
    .toHaveProperty('username', 'ユーザー名');
}));

describe('selectInboxUri', () => test('loads and returns inbox URI', async () => {
  const account = new RemoteAccount({
    repository,
    inbox: { uri: new URI({ repository, uri: 'https://remote.xn--kgbechtv/' }) },
    person: new Person({
      repository,
      username: '',
      host: 'finger.remote.xn--kgbechtv'
    }),
    publicKey: {
      uri: new URI({ repository, uri: '' }),
      publicKeyPem: ''
    },
    uri: new URI({ repository, uri: '' })
  });

  await repository.insertRemoteAccount(account);
  account.inbox.uri = new URI({ repository, id: account.inbox.uri.id });

  await expect(account.selectInboxUri())
    .resolves
    .toHaveProperty('uri', 'https://remote.xn--kgbechtv/');
}));

describe('selectPublicKeyUri', () => test('loads and returns inbox URI', async () => {
  const account = new RemoteAccount({
    repository,
    inbox: { uri: new URI({ repository, uri: '' }) },
    person: new Person({
      repository,
      username: '',
      host: 'finger.remote.xn--kgbechtv'
    }),
    publicKey: {
      uri: new URI({ repository, uri: 'https://remote.xn--kgbechtv/' }),
      publicKeyPem: ''
    },
    uri: new URI({ repository, uri: '' })
  });

  await repository.insertRemoteAccount(account);
  account.publicKey.uri = new URI({ repository, id: account.publicKey.uri.id });

  await expect(account.selectPublicKeyUri())
    .resolves
    .toHaveProperty('uri', 'https://remote.xn--kgbechtv/');
}));

describe('create', () => {
  test('creates and resolves with an account', async () => {
    const account = await RemoteAccount.create(
      repository,
      'name of user',
      'finger.remote.xn--kgbechtv',
      'https://remote.xn--kgbechtv/@name%20of%20user',
      { uri: 'https://remote.xn--kgbechtv/@name%20of%20user/inbox' },
      {
        uri: 'https://remote.xn--kgbechtv/@name%20of%20user#key',
        publicKeyPem: `-----BEGIN RSA PUBLIC KEY-----

+DHYFaVXQy60WzCEFjiQPZ8XdNQKvDyjKwIDAQAB
-----END RSA PUBLIC KEY-----
`
      });

    expect(account).toBeInstanceOf(RemoteAccount);
    expect(account).toHaveProperty('repository', repository);
    expect(account).toHaveProperty(['person', 'repository'], repository);
    expect(account).toHaveProperty(['person', 'username'], 'name of user');
    expect(account).toHaveProperty(['person', 'host'],
      'finger.remote.xn--kgbechtv');
    expect(account).toHaveProperty(['uri', 'uri'],
      'https://remote.xn--kgbechtv/@name%20of%20user');
    expect(account).toHaveProperty(['publicKey', 'uri', 'uri'],
      'https://remote.xn--kgbechtv/@name%20of%20user#key');
    expect(account).toHaveProperty(['publicKey', 'publicKeyPem'], `-----BEGIN RSA PUBLIC KEY-----

+DHYFaVXQy60WzCEFjiQPZ8XdNQKvDyjKwIDAQAB
-----END RSA PUBLIC KEY-----
`);

    await expect(repository.selectRemoteAccountByUsernameAndNormalizedHost('name of user',  'finger.remote.xn--kgbechtv'))
      .resolves
      .toHaveProperty(['publicKey', 'publicKeyPem'], `-----BEGIN RSA PUBLIC KEY-----

+DHYFaVXQy60WzCEFjiQPZ8XdNQKvDyjKwIDAQAB
-----END RSA PUBLIC KEY-----
`);
  });

  test('rejects if username is invalid', () => expect(RemoteAccount.create(
    repository,
    '',
    'finger.remote.xn--kgbechtv',
    'https://remote.xn--kgbechtv/@%E3%83%A6%E3%83%BC%E3%82%B6%E3%83%BC%E5%90%8D',
    { uri: 'https://remote.xn--kgbechtv/@%E3%83%A6%E3%83%BC%E3%82%B6%E3%83%BC%E5%90%8D/inbox' },
    {
      uri: 'https://remote.xn--kgbechtv/@%E3%83%A6%E3%83%BC%E3%82%B6%E3%83%BC%E5%90%8D#key',
      publicKeyPem: `-----BEGIN RSA PUBLIC KEY-----

+DHYFaVXQy60WzCEFjiQPZ8XdNQKvDyjKwIDAQAB
-----END RSA PUBLIC KEY-----
`
    })).rejects.toBeInstanceOf(Error));
});
