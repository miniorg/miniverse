...
describe('getUri', () => {
  test('loads and returns URI of local key', async () => {
    const owner = new LocalPerson(repository, null, {
      admin: true,
      privateKeyPem: '',
      salt: '',
      serverKey: '',
    ...
    await repository.insertLocalPerson(owner);

    const key = new Key({
      owner: new Person(repository, owner.id),
      repository
    });
  ...
  test('loads and returns URI of remote key', async () => {
    const owner = new RemotePerson(repository, null, {
      inbox: { uri: new URI(repository, null, { uri: '' }) },
      publicKey: {
        uri: new URI(repository, null, { uri: 'https://OwNeR.xn--kgbechtv/' }),
        publicKeyPem: ''
      },
      uri: '',
      username: '',
    ...
    await repository.insertRemotePerson(owner);
...
content-type: application/activity+json`,
  ...
  test('loads person and returns true if signature is valid', async () => {
    const owner = new RemotePerson(repository, null, {
      inbox: { uri: new URI(repository, null, { uri: '' }) },
      publicKey: {
...
`
  ...
  test('loads person and returns false if signature is valid', async () => {
    const owner = new RemotePerson(repository, null, {
      inbox: { uri: new URI(repository, null, { uri: '' }) },
      publicKey: {
        uri: new URI(repository, null, {
