...
function parse({ id, uri, attributed_to_id, content }) {
  return {
    id,
    uri,
    attributedTo: new Person(this, attributed_to_id),
    content
  };
}
...
export default {
  deleteNoteByUriAndAttributedTo(uri, attributedTo) {
    return this.pg.query({
      name: 'deleteNoteByUriAndAttributedTo',
      text: 'DELETE FROM notes WHERE uri = $1 AND attributed_to_id = $2',
      values: [uri, attributedTo.id]
    });
  },
