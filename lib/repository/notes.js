...
function parse({ id, uri, attributed_to_id, content }) {
  const note = new Note({
    id,
    uri,
    attributedTo: new Person({ repository: this, id: attributed_to_id }),
    content
  });
...
export default {
  async insertNote(note) {
    const { rows } = await this.pg.query({
      name: 'insertNote',
      text: 'INSERT INTO notes (uri, attributed_to_id, content) VALUES ($1, $2, $3) RETURNING id',
      values: [note.uri, note.attributedTo.id, note.content]
    });
