/*
  Copyright (C) 2018  Miniverse authors

  This program is free software: you can redistribute it and/or modify
  it under the terms of the GNU Affero General Public License as published by
  the Free Software Foundation, version 3 of the License.

  This program is distributed in the hope that it will be useful,
  but WITHOUT ANY WARRANTY; without even the implied warranty of
  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
  GNU Affero General Public License for more details.

  You should have received a copy of the GNU Affero General Public License
  along with this program.  If not, see <https://www.gnu.org/licenses/>.
*/

import Cookie from '../cookie';
import Follow from '../follow';
import LocalPerson from '../local_person';
import Note from '../note';
import repository from '../test_repository';

const privateKeyPem = `-----BEGIN RSA PRIVATE KEY-----

OyJRYe+sFKZ6lXqnwdWuTrxTNucFuhw+6BVyzNn6lI5cNXLr1reH
-----END RSA PRIVATE KEY-----
`;

describe('selectLocalPersonByDigestOfCookie', () =>
  test('resolves with null if not found', () => {
    const digest = Buffer.from([]);
    return expect(repository.selectLocalPersonByDigestOfCookie(digest))
      .resolves
      .toBe(null);
  }));

describe('selectLocalPersonByUsername', () =>
  test('resolves with null if not found', () =>
    expect(repository.selectLocalPersonByUsername('')).resolves.toBe(null)));

test('delivers objects to inboxses', async () => {
  const attributedTo = new LocalPerson(repository, null, {
    admin: true,
    username: '',
    host: null,
    privateKeyPem,
    salt: '',
    serverKey: '',
    storedKey: ''
  });

  await repository.insertLocalPerson(attributedTo);

  const note = new Note(repository, null, { attributedTo, content: '' });
  await repository.insertNote(note);

  let resolveMessage;
  const asyncMessage = new Promise(resolve => resolveMessage = resolve);
  await repository.subscribe(repository.getInboxChannel(attributedTo),
    (channel, message) => resolveMessage(JSON.parse(message)));

  await repository.insertIntoInboxes([attributedTo], note);

  await Promise.all([
    expect(asyncMessage).resolves.toHaveProperty('type', 'Note'),
    expect(asyncMessage).resolves.toHaveProperty('content', ''),
    repository.selectRecentNotesFromInbox(attributedTo).then(notes =>
      expect(notes[0]).toHaveProperty('id', note.id))
  ]);
});

test('inserts cookie and allows to query person with its digest', async () => {
  const digest = Buffer.from('digest');
  const cookie = new Cookie({
    person: new LocalPerson(repository, null, {
      admin: true,
      username: 'username',
      host: null,
      privateKeyPem,
      salt: Buffer.from('salt'),
      serverKey: Buffer.from('serverKey'),
      storedKey: Buffer.from('storedKey')
    }),
    digest
  });

  await repository.insertLocalPerson(cookie.person);
  await repository.insertCookie(cookie);

  const queried = await repository.selectLocalPersonByDigestOfCookie(digest);
  const queriedProperties = await queried.get();

  expect(queried).toHaveProperty('repository', repository);
  expect(queried).toHaveProperty('id', cookie.person.id);
  expect(queriedProperties).toHaveProperty('admin', true);
  expect(queriedProperties).toHaveProperty('username', 'username');
  expect(queriedProperties).toHaveProperty('host', null);
  expect(queriedProperties).toHaveProperty('privateKeyPem', privateKeyPem);
  expect(Buffer.from('salt').equals(queriedProperties.salt)).toBe(true);
  expect(Buffer.from('serverKey').equals(queriedProperties.serverKey)).toBe(true);
  expect(Buffer.from('storedKey').equals(queriedProperties.storedKey)).toBe(true);
});

test('inserts follow and allows to query person with its object', async () => {
  const actor = new LocalPerson(repository, null, {
    admin: true,
    privateKeyPem,
    salt: Buffer.from('salt'),
    serverKey: Buffer.from('serverKey'),
    storedKey: Buffer.from('storedKey'),
    username: '行動者',
    host: null
  });

  const object = new LocalPerson(repository, null, {
    admin: true,
    privateKeyPem: '',
    salt: '',
    serverKey: '',
    storedKey: '',
    username: '被行動者',
    host: null
  });

  await Promise.all([
    repository.insertLocalPerson(actor),
    repository.insertLocalPerson(object)
  ]);

  const inserted = new Follow(repository, null, { actor, object });
  await repository.insertFollow(inserted);

  const [queried] = await repository.selectLocalPersonsByFollowee(object);
  const queriedProperties = await queried.get();

  expect(queried).toHaveProperty('repository', repository);
  expect(queried).toHaveProperty('id', actor.id);
  expect(queriedProperties).toHaveProperty('admin', true);
  expect(queriedProperties).toHaveProperty('username', '行動者');
  expect(queriedProperties).toHaveProperty('host', null);
  expect(queriedProperties).toHaveProperty('privateKeyPem', privateKeyPem);
  expect(Buffer.from('salt').equals(queriedProperties.salt)).toBe(true);
  expect(Buffer.from('serverKey').equals(queriedProperties.serverKey)).toBe(true);
  expect(Buffer.from('storedKey').equals(queriedProperties.storedKey)).toBe(true);
});

test('inserts person and allows to load its properties', async () => {
  const person = new LocalPerson(repository, null, {
    admin: true,
    username: 'username',
    host: null,
    privateKeyPem,
    salt: Buffer.from('salt'),
    serverKey: Buffer.from('serverKey'),
    storedKey: Buffer.from('storedKey')
  });

  await repository.insertLocalPerson(person);

  const queriedProperties = await repository.loadLocalPerson(person);

  expect(queriedProperties).toHaveProperty('admin', true);
  expect(queriedProperties).toHaveProperty('username', 'username');
  expect(queriedProperties).toHaveProperty('host', null);
  expect(queriedProperties).toHaveProperty('privateKeyPem', privateKeyPem);
  expect(Buffer.from('salt').equals(queriedProperties.salt)).toBe(true);
  expect(Buffer.from('serverKey').equals(queriedProperties.serverKey)).toBe(true);
  expect(Buffer.from('storedKey').equals(queriedProperties.storedKey)).toBe(true);
});

test('inserts person and allows to query one by its person\'s username', async () => {
  const person = new LocalPerson(repository, null, {
    admin: true,
    username: 'username',
    host: null,
    privateKeyPem,
    salt: Buffer.from('salt'),
    serverKey: Buffer.from('serverKey'),
    storedKey: Buffer.from('storedKey')
  });

  await repository.insertLocalPerson(person);

  const queried = await repository.selectLocalPersonByUsername('username');
  const queriedProperties = await queried.get();

  expect(queried).toHaveProperty('repository', repository);
  expect(queried).toHaveProperty('id', person.id);
  expect(queriedProperties).toHaveProperty('admin', true);
  expect(queriedProperties).toHaveProperty('username', 'username');
  expect(queriedProperties).toHaveProperty('host', null);
  expect(queriedProperties).toHaveProperty('privateKeyPem', privateKeyPem);
  expect(Buffer.from('salt').equals(queriedProperties.salt)).toBe(true);
  expect(Buffer.from('serverKey').equals(queriedProperties.serverKey)).toBe(true);
  expect(Buffer.from('storedKey').equals(queriedProperties.storedKey)).toBe(true);
});
