...
test('inserts note and allows to query by its mentions', async () => {
  ...
  const mentions = await repository.selectMentionsIncludingPersonsOuterRemoteAccountsOuterUrisByNoteId(note.id);
  expect(mentions[0]).toHaveProperty(['href', 'id'], attributedTo.id);
});
