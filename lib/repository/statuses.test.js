...
test('inserts note and allows to query its status by the id of actor attributed to', async () => {
  ...
  const [queried] = await repository.selectRecentStatusesIncludingExtensionsByActorId(note.status.actorId);

  expect(queried).toHaveProperty('repository', repository);
  expect(queried).toHaveProperty('id', note.status.id);
  expect(queried).toHaveProperty('published', note.status.published);
  expect(queried).toHaveProperty('actorId', note.status.actorId);
});
...
test('inserts note and allows to put its status into inbox and query it', async () => {
  ...
  await repository.insertIntoInboxes([note.status.actor.account], note.status);

  const [queried] =
    await repository.selectRecentStatusesIncludingExtensionsAndActorsFromInbox(
      note.status.actor.account.id);
