...
export default {
  ...
  async selectLocalAccountsByFollowee({ id }) {
    const { rows } = await this.pg.query({
      ...
      text: 'SELECT local_accounts.* FROM local_accounts JOIN follows ON local_accounts.id = follows.actor_id WHERE follows.object_id = $1',
      values: [id]
    });
  ...
  async selectLocalAccountByUsername(username) {
    const { rows } = await this.pg.query({
        ...
        text: 'SELECT local_accounts.* FROM local_accounts JOIN persons USING(id) WHERE persons.username = $1 AND lower(persons.host) = \'\'',
        values: [username]
      });
