...
function parse(person, { uri, inbox_uri_id, key_uri_id, public_key_pem }) {
...
export default {
  async insertRemoteAccount(account) {
    const { rows } = await this.pg.query({
      ...
      text: 'SELECT * FROM insert_remote_account($1, $2, $3, $4, $5, $6) AS (person_id BIGINT, inbox_uri_id BIGINT, key_uri_id BIGINT)',
      values: [
    ...
    account.person.id = rows[0].person_id;
  ...
  async selectRemoteAccountByUri(uri) {
    const { rows } = await this.pg.query({
      name: 'selectRemoteAccountByUri',
      text: 'SELECT remote_accounts.* FROM remote_accounts WHERE remote_accounts.uri = $1',
      values: [uri]
    });
