...
function parse(person, { id, uri, inbox_uri, key_uri, public_key_pem }) {
...
export default {
  async loadRemoteAccount(account) {
    if (this.loadeds.has(account)) {
      return account;
    }

    const { rows } = await this.pg.query({
      name: 'loadRemoteAccount',
      text: 'SELECT * FROM remote_accounts WHERE id = $1',
      values: [account.id]
    });

    if (rows[0]) {
      account.person = new Person({ repository: this, id: rows[0].person_id });
      account.uri = rows[0].uri;
      account.inbox = { uri: rows[0].inbox_uri };
      account.publicKey =
        { uri: rows[0].key_uri, publicKeyPem: rows[0].public_key_pem };
  ...
  async selectRemoteAccountByKeyUri(uri) {
    const { rows } = await this.pg.query({
      ...
      text: 'SELECT * FROM remote_accounts WHERE key_uri = $1',
      values: [uri]
    });

    if (rows[0]) {
      const account = parse(
        new Person({ repository: this, id: rows[0].person_id }), rows[0]);
