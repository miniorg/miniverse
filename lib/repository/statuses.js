...
function parseExtension({ id, announce_object_id, note_summary, note_content }) {
...
export default {
  ...
  async selectRecentStatusesIncludingExtensionsByPersonId(personId) {
    const { rows } = await this.pg.query({
      ...
      text: 'SELECT statuses.*, announces.object_id AS announce_object_id, notes.summary AS note_summary, notes.content AS note_content FROM statuses LEFT OUTER JOIN announces USING (id) JOIN notes ON COALESCE(announces.object_id, statuses.id) = notes.id WHERE statuses.person_id = $1 ORDER BY statuses.id DESC',
      values: [personId]
    });
