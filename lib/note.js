...
export default class Note extends Relation {
  ...
  static async create(repository, person, content, { published = null, uri, inReplyTo = null, inReplyToUri = null, summary = null, mentions } = {}) {
    const note = new this({
      repository,
      status: new Status({
        repository,
  ...
  static async fromParsedActivityStreams(repository, object, givenAttributedTo) {
    ...
    const [
      published, attributedTo, to, [inReplyTo, inReplyToUri],
      summary, content, mentions
    ] = await Promise.all([
      object.getPublished(),
    ...
    return this.create(
      ...
      attributedTo,
      content,
      { published, uri, inReplyTo, inReplyToUri, summary, mentions });
  }
}
